...
var deltaTimes:PackedFloat32Array = [0, 0, 0];
...
func missed(props:Dictionary, _misc:Dictionary, isEnemy:bool, ignoreMissCount:bool = false):
	var deltaTime = deltaTimes[0];
	if (!isEnemy):
		if (Data.noteTypeLabels.has(props.noteType)):
			...
			match (noteType):
				"Note", "Good":
					if (voices):
						voices.volume_db = -72;
					# if (deltaTime >= BeatHandler.stepSecs):
					health -= 0.075;
					updateScore(0, true);
...
func hit(props:Dictionary, _misc:Dictionary, isEnemy:bool):
	if (voices):
		voices.volume_db = 0;
	if (!isEnemy):
		if (Data.noteTypeLabels.has(props.noteType)):
...
func hold(_props:Dictionary, _misc:Dictionary, isEnemy:bool):
	var deltaTime = deltaTimes[1];
	if (deltaTime >= BeatHandler.stepSecs):
		deltaTime = 0;
		if (!isEnemy):
			health += 0.0075;
...
func pressed(_key:Key):
	...
	if (key in range(playerStrums.size())):
		...
		if (holds.size() > 0):
			for holdN in holds:
				if (weakref(holdN).get_ref()):
					strum.confirmLoop();
					stage.bfConfirmLoop(key, holdN.properties.alt);
					hold(holdN.properties.duplicate(true), holdN.misc.duplicate(true), holdN.properties.col < 4);
					holdN.holdUpdate();
				else:
					holds.erase(hold);
...
func updateHealthBar():
	...
	if (health > 2):
		...
		hud.updHealth(health);
	if (health < 0.4):
		if (enemyIcon.animation != "2"):
			enemyIcon.play("2");
		if (playerIcon.animation != "1"):
			playerIcon.play("1");
	elif (health > 1.6):
		if (enemyIcon.animation != "1"):
			enemyIcon.play("1");
		if (playerIcon.animation != "2"):
			playerIcon.play("2");
	else:
		if (enemyIcon.animation != "0"):
			enemyIcon.play("0");
		if (playerIcon.animation != "0"):
			playerIcon.play("0");
...
#	var j:bool = true;
#	var list:Array[Dictionary] = get_property_list();
#	for dict in list:
...
#			var val = module.get(property);
#			if (val):
#				return val;

func global_call(func_name:String, func_data:Array):
	for module in modules.values():
		if (module.has_method(func_name)):
			module.get(func_name).callv(func_data);

func registerModule(className:String, modName:String, code:String, autoload:bool):
	# print("aa");
	print(modName);
	var script := GDScript.new();
	script.set_source_code(code);
	script.reload();
	var module = ClassDB.instantiate(className);
	module.set_script(script);
	module.set_name(modName);
	if (autoload):
		if (!Modules.globals.has(modName)):
			Modules.add_child(module);
			module.set_owner(Modules);
			Modules.globals[modName] = module;
	else:
		add_child(module);
...
# 	rsgTimer.connect("timeout", rsgUpdate);
# 	InputHandler.connect("justPressed", justPressed);
# 	InputHandler.connect("pressed", pressed);
# 	InputHandler.connect("justReleased", justReleased);
# 	SceneTransition.connect("finished", _sceneLoaded);
# 	stats.connect("finishedStats", statsFinished);
...
# 	ModuleLoader.read(registerModule);
			...
			if (note.misc.time <= instPos && col in range(enemyStrums.size())):
				...
				if (!holds.has(note)):
					...
					if (noteDead):
						...
						holds.erase(note);
		if (weakref(note).get_ref()):
			if (note.position.y + note.holdEnd.offset.y + sizeAdd / 2.0 <= 0):
...
func stepHit(steps:int):
	...
	if (stage):
		stage.stepHit(steps);
	global_call("stepHit", [steps]);
	pass;

func beatHit(beats:int):
	if (stage):
		stage.beatHit(beats);
	global_call("beatHit", [beats]);
	pass;
...
func sectionHit(sections:int):
	if (focuses.size() > 0 && stage):
		...
		focuses.pop_front();
	global_call("sectionHit", [sections]);
	pass;
