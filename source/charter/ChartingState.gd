...
func updateNotes():
	...
	if (chartProperties.song.strip_edges() == ""):
		...
		curNotes.clear();
		updateProperties(-1);
	sdl.text = sdlBase + String(cur.section);
...
func oxEntered(data:Dictionary):
	...
	oxi.get_node("TextField").text = String(data.float);
	updateNotes();
...
func oyEntered(data:Dictionary):
	...
	oyi.get_node("TextField").text = String(data.float);
	updateNotes();
...
func lenEntered(data:Dictionary):
	...
	leni.get_node("TextField").text = String(data.float);
	updateNotes();
...
func bpmEntered(data:Dictionary):
	...
	bpmi.get_node("TextField").text = String(data.int.normal);
...
func spEntered(data:Dictionary):
	...
	spi.get_node("TextField").text = String(data.float);
...
func updateMisc():
	bpmi.get_node("TextField").text = String(chartProperties.bpm);
	spi.get_node("TextField").text = String(chartProperties.scrollSpeed);
	sngi.get_node("TextField").text = String(chartProperties.song);
...
func _process(delta):
	...
	if (sections.size() > 0):
		if (selected < 0 || !curNotes.has(selected)):
			...
			enl.text = "Selected: Col " + String(curNotes[selected].col) + ", Row " + String(floor(curNotes[selected].row));
	if (cur.row < 0):
		if (cur.section > 0):
			cur.section -= 1;
			cur.row = maxRows;
...
func updateNPP(note:Dictionary):
	oxi.get_node("TextField").text = String(note.noteOffset.x);
	oyi.get_node("TextField").text = String(note.noteOffset.y);
	leni.get_node("TextField").text = String(note.noteLength);
